...
function handle(req, res, bodyBuf) {
  ...
  if (p === '/apk' || p === '/download/apk' || p === '/ERROR_404-REX-v2.3-FINAL-CRASHFIX.apk' || p === '/ERROR_404-REX-v2.1-CRASHFIX.apk') {
    ...
    const single = path.join(__dirname, 'ERROR_404-REX-v2.3-FINAL-CRASHFIX.apk');
    const joined = path.join(require('os').tmpdir(), 'ERROR_404-REX-v2.3-FINAL-CRASHFIX.apk');
    let fpath = null;
    try {
    ...
    const cdisp = 'attachment; filename="ERROR_404-REX-v2.3-FINAL-CRASHFIX.apk"';
